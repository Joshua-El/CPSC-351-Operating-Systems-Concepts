...
string recvFileName()
{
     ...
     * used for holding the message received from the sender.
         */
        fileNameMsg fileToReceive;
        /* TODO: Receive the file name using msgrcv() */
  			if(msgrcv(msqid, &fileToReceive, sizeof(fileNameMsg) - sizeof(long), FILE_NAME_TRANSFER_TYPE, 0) < 0)
        {
          perror("msgrcv");
          exit(-1);
        }
    ...
    /* TODO: return the received file name */
        fileName =  fileToReceive.fileName;
        return fileName;
}
 /**
 ...
 */
 void init(int& shmid, int& msqid, void*& sharedMemPtr)
 {
   ...
   if(sharedMemPtr < (void*)0)  // maybe (void*) 0
   {
       perror("shmat");
       exit(1);
   }
