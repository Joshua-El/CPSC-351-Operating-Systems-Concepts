 ...
 */
 void init(int& shmid, int& msqid, void*& sharedMemPtr)
 {
     ...
     if(shmid < 0)
   {
     ...
     /* Store the IDs and the pointer to the shared memory
     region in the corresponding function parameters */
...
unsigned long sendFile(const char* fileName)
{
	...
	while(!feof(fp))
	{
		 ...
		 * read in numBytesSent to record how many bytes were actually send.
 		 */
     if((sndMsg.size = fread(sharedMemPtr, 1, SHARED_MEMORY_CHUNK_SIZE,fp)) < 0)
     {
       perror("fread");
       exit(-1);
    ...
    message nothing;
    nothing.size = 0;
    nothing.mtype = SENDER_DATA_TYPE;
    if(msgsnd(msqid, &nothing, sizeof(message) - sizeof(long), 0) < 0)
    {
      perror("msgsnd");
      exit(-1);
    }
...
void sendFileName(const char* fileName)
{
  ...
  for(int i = 0; i < sizeof(fileName)+4; i++)
  {
    fileToSend.fileName[i] = fileName[i];
  }
	/* TODO: Send the message using msgsnd */
  if(msgsnd(msqid, &fileToSend, sizeof(fileNameMsg) - sizeof(long), 0) < 0)
  {
    perror("msgsnd");
    exit(-1);
  }
